                  ...
                  {profile.work_experiences.map((we, i) => (
                    weEditing === we.id ? (
                      ...
                      <div key={we.id} className="flex gap-4 py-[18px] border-t border-slate-100 first:border-t-0 first:pt-0 group">
                        ...
                        <RowActions onEdit={() => startEditWE(we)} onRemove={() => handleWEDelete(we.id)} />
                      </div>
                    )
                  ))}
                </div>
                        ...
                        eduEditing === ed.id ? (
                          ...
                          <div key={ed.id}>
                            ...
                            <div className="flex items-start gap-3 group">
                              ...
                              <RowActions onEdit={() => startEditEdu(ed)} onRemove={() => handleEduDelete(ed.id)} />
                            </div>
                          </div>
                        )
                      ))}
...
function RowActions({ onEdit, onRemove }: { onEdit: () => void; onRemove: () => void }) {
  return (
    <div className="flex items-center gap-0.5 shrink-0 self-start opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
      ...
      <button onClick={onRemove} aria-label="Remove" title="Remove"
